...
`defineConfig` is exported from `@bedrock-rbx/bedrock` and accepts either a plain
...
- c12 and arktype are runtime dependencies of `@bedrock-rbx/bedrock`. ADR-008's
  zero-runtime-dependencies rule is scoped to `@bedrock-rbx/open-cloud` and
