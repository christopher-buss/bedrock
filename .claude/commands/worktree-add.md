...
<step_0b>
  <description>Detect git hosting provider (only needed if argument is an issue URL)</description>
...
<detect_provider>
  ...
  <purpose>Detect git hosting provider and select appropriate CLI tool for issue lookup</purpose>
</step_0b>
  ...
  <step_2_5>
    ...
    <fetch_issue_details>
      <tool>Use $GIT_HOST_CLI from step_0b (e.g., gh issue view, glab issue view)</tool>
      <method>get issue details</method>
      <parameters>issue_number</parameters>
    </fetch_issue_details>
...
- Offers Quick or Full setup mode - Quick just creates the worktree, Full does
  everything
- Automatically detects and uses your current IDE (VS Code, VS Code Insiders,
  Cursor, Zed, etc.) in Full mode
- Creates VS Code-specific tasks.json only for VS Code variants (auto-starts
  Claude on folder open)
...
- Requires CLI tool (gh for GitHub, glab for GitLab) when using issue URLs
