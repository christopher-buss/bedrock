...
the release, and the GitHub Release on that `v` tag is generated by
`changelogithub` from the `feat`/`fix`/`perf` commits in the range. Commit
subjects are therefore consumer-facing copy twice over: in the PR-title
changelog entry and in the release notes.
